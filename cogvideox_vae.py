...
# Temporary fix for PIL import issues and cv2 alternatives
try:
    from kitchen_env_wrappers import readGif
except (ImportError, AttributeError):
    def readGif(filename):
        """
        Fallback readGif function
        """
        from PIL import Image, ImageSequence
...
# OpenCV alternative: use PIL for resizing
try:
    import cv2
    USE_CV2 = True
except ImportError:
...
class VAETester:
    def __init__(self, model_path="THUDM/CogVideoX-5b-I2V", device="cuda", dtype=torch.float16, lora_path=None):
        """
    ...
    def preprocess_video_for_vae(self, frames, target_size=(480, 480), target_frames=16):
        ...
        # Rearrange dimensions: [T, H, W, C] -> [C, T, H, W] -> [1, C, T, H, W]
...
def main():
    ...
    # Initialize tester
    try:
        tester = VAETester(args.model_path, args.device, args.dtype, lora_path=args.lora_path)
    except Exception as e:
