...
class CalvinPlayTable:
    """
    Dreamer-style CALVIN single-task wrapper (following wrappers.MetaWorld).
    - Interaction object is internal PlayTableSimEnv (created via get_env(...))
    - Single task: specified by task_name, sparse reward (task completion=1, otherwise 0)
    - Observation: image (static camera), state (robot_obs vector), flags
    - Termination: success or reaching step limit; success written to info['success']
    """
    def __init__(self, dataset_path: Optional[str],
                 ...
                 show_gui: bool = False,
                 scene: Optional[str] = None):  # New scene parameter
       ...
       # dataset_path optional: if provided, try to load from dataset's merged_config; otherwise use package built-in default config
        if dataset_path:
        ...
        # Step length and rendering
        self._size = size
        self._ep_len = ep_len
        self._t = 0
        self._start_info = None

        # Action space (CALVIN Cartesian + gripper convention: dx,dy,dz, rx,ry,rz, gripper)
        self._action_space = spaces.Box(low=-1.0, high=1.0, shape=(7,), dtype=np.float32)

        # Construct one observation to define obs_space
    ...
    def reset(self, options=None):
        """
        options optional: {"robot_obs": np.ndarray, "scene_obs": np.ndarray} for fixed initial state
        """
    ...
    def step(self, action: np.ndarray):
        ...
        out = {
            ...
            "is_terminal": bool(success),  # Dreamer commonly uses is_terminal to indicate "natural termination" (success)
            "state": state,
        }
        info = dict(info)
    ...
    def _render_rgb(self, obs):
        # Directly use underlying render to get static camera view, then resize to size
    ...
    def _vectorize_robot_state(self, obs) -> np.ndarray:
        """
        Convert CALVIN's robot_obs(dict) to flat vector, compatible with Dreamer.
        Prefer using 'robot_state_full', otherwise concatenate numeric fields.
...
class MetaWorldGC:
    def __init__(self, name, sparse_interval=None, alpha=1, beta_scale =0.0001, device=None, crop=None, seed=None, action_repeat=1, size=(64, 64), camera=None, text_string=None, video_path=None, 
